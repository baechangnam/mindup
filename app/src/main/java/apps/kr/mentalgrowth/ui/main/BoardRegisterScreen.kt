...
@Composable
fun BoardRegisterScreen(navController: NavController, code : String,  idx: String? = null, viewModel: MainViewModel = viewModel()) {
    // 제목, 내용 입력 상태
        ...
        topBar = {
            TopAppBar(
                ...
                Box(
                    ...
                    // ✅ 중앙 타이틀 (하트 포함)
                    CommonView.TitleWithHearts("마음터치 등록")
        ...
        content = { innerPadding ->
            Column(
                ...
                OutlinedTextField(
                    ...
                    label = { Text("제목", style = MaterialTheme.typography.subtitle1,) },
                ...
                OutlinedTextField(
                    ...
                    label = { Text("내용", style = MaterialTheme.typography.subtitle1) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(200.dp),
                    textStyle = TextStyle(
                ...
                Row(
                    ...
                    Text("자기평가: ", modifier = Modifier.padding(end = 8.dp), style = MaterialTheme.typography.body2)
                ...
                OutlinedTextField(
                    ...
                    label = { Text("링크", style = MaterialTheme.typography.subtitle1) },
                    modifier = Modifier.fillMaxWidth()
                )
                ...
                when {
                    rating >= i -> {
                        Icon(
                            ...
                            contentDescription = "Full Heart $i",
                            tint = Color.Red,
                    ...
                    rating >= i - 0.5f -> {
                        Image(
                            painter = painterResource(id = R.drawable.heart_half_svgrepo_com),
                        ...
                        Icon(
                            ...
                            contentDescription = "Empty Heart $i",
                            tint = Color.Red,
                            modifier = Modifier.size(28.dp)
                        )
                    }
...
fun UploadBoardButton(
    ...
    onTitleEmpty: () -> Unit,
    code: String,    addr: String,    rating: Float , idx : String
) {
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        Button(
            contentPadding = PaddingValues(vertical = 15.dp, horizontal = 5.dp),
            onClick = {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        ...
                        val memIds = createPartFromString(memId)
                        val addr = createPartFromString(addr)
                        val code = createPartFromString(code)
                        val rating = createPartFromString(rating.toString())
                        val group_id = createPartFromString(class_group_id)
                        val pid = createPartFromString(pid)
                        // 이미지가 선택되었다면 파일 파트 생성, 없으면 null
                        val imagePart =
                            imageUri?.let { prepareFilePartAllFile(context, it, "image") }

                        if (idx.isNullOrBlank()){
                            val response = apiService.uploadBoardTouch(
                                pid = pid,
                                title = titlePart,
                                content = contentPart,
                                reg_id = memIds,
                                addr = addr, code = code, rating = rating, group_id = group_id,
                                image = imagePart,
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                if (response.isSuccessful && response.body()?.flag?.flag == "1") {
                                    launch(Dispatchers.Main) {
                                        Toast.makeText(context, "등록 완료되었습니다.", Toast.LENGTH_SHORT)
                                            .show()
                                        onUploadSuccess()
                                    }
                                } else {
                            ...
                            val idx = createPartFromString(idx)
                            val response = apiService.upDateBoardTouch(
                                title = titlePart,
                                content = contentPart,
                                reg_id = memIds,
                                addr = addr, code = code, rating = rating, group_id = group_id,
                                image = imagePart,idx= idx
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                if (response.isSuccessful && response.body()?.flag?.flag == "1") {
                                    launch(Dispatchers.Main) {
                                        Toast.makeText(context, "수정 완료되었습니다.", Toast.LENGTH_SHORT)
                                            .show()
                                        onUploadSuccess()
                                    }
                                } else {
...
fun prepareFilePart(context: Context, uri: Uri, partName: String): MultipartBody.Part? {
    // 임시 파일 생성 (파일 이름은 원하는 대로 지정)
    val fileName = "caddy_${System.currentTimeMillis()}.jpg"
    val file = File(context.cacheDir, fileName)
    try {
        // InputStream을 열고, Okio의 확장 함수를 사용하여 파일에 기록
